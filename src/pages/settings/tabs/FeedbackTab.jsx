...
// User Feedback — list + detail + delete
...
function DetailDrawer({ itemId, onClose, onDeleted }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const { data, ok } = await apiFetch(`supports/feedback/${itemId}/`)
  ...
  const handleDelete = async () => {
    setDeleting(true)
    const { ok } = await apiFetch(`supports/feedback/${itemId}/delete/`, { method: 'DELETE' })
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-[#eff6ff] flex items-center justify-center shrink-0">
                  <span className="text-[#3b82f6] font-bold text-[15px]">{detail.first_name?.[0]}{detail.last_name?.[0]}</span>
                </div>
                <div>
                  <p className="text-[#0f172b] font-bold text-[15px]">{detail.first_name} {detail.last_name}</p>
                  <p className="text-[#62748e] text-[12px]">{new Date(detail.created_at).toLocaleDateString('en-AU', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: IconMail,  label: 'Email',    val: detail.email },
                  { icon: IconPhone, label: 'Phone',    val: detail.phone },
                  { icon: IconGlobe, label: 'Country',  val: detail.country },
                  { icon: IconGlobe, label: 'Language', val: detail.language },
              ...
              <div>
                ...
                <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-[10px] p-4">
                  <p className="text-[14px] text-[#314158] leading-[22px]">{detail.message}</p>
                </div>
              </div>
            </div>
...
export default function FeedbackTab() {
  ...
  const load = async (pageNum = 1) => {
    setLoading(true)
    setLoadErr(null)
    const { data, ok } = await apiFetch(`supports/feedback/?page=${pageNum}`)
    if (ok && data) {
      ...
      setLoadErr('Failed to load feedback. Please refresh.')
    }
    setLoading(false)
  }
  ...
  return (
    <>
      ...
      <div className="max-w-[800px] flex flex-col gap-4">
        <p className="text-[#62748e] text-[13px]">{totalCount} feedback submission{totalCount !== 1 ? 's' : ''}</p>

        <div className="bg-white border border-[#e2e8f0] rounded-[14px] overflow-hidden">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              <p className="text-[#90a1b9] text-[14px]">No feedback submissions yet.</p>
            </div>
          ) : items.map((item, idx) => (
            <button
              key={item.id}
            ...
            >
              ...
              <span className="text-[11px] text-[#90a1b9] shrink-0">
        ...
        {(prevUrl || nextUrl) && (
          <div className="flex items-center justify-between pt-1">
            <button
              onClick={() => load(page - 1)}
              disabled={!prevUrl}
              className="px-4 py-2 rounded-[8px] border border-[#e2e8f0] text-[#314158] text-[13px] font-semibold hover:bg-[#f8fafc] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              ← Previous
            </button>
